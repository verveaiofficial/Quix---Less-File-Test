...
export const useUsageStore = create<any>((set, get) => ({
  usage: readUsage(),
  limitFor: (m: string) => { const sess = useAuthStore.getState().session; if (!sess) return m === "thinking" ? GUEST_THINKING_LIMIT : 0; return LIMITS[m] ?? 30; },
...
export async function askGeminiStream(model: string, prompt: string, opts: { search?: boolean; nativeThoughts?: boolean; attachments?: any[] }, h: { onThoughts: (t: string) => void; onText: (t: string) => void; onDone: (r: GeminiResult) => void }): Promise<void> {
  ...
  try {
    ...
    if (!res.ok || !res.body) { const d = await res.json().catch(() => null); throw new Error(`Gemini ${GEMINI_MODEL} failed (${res.status}): ${d?.error?.message || "unknown"}`); }
