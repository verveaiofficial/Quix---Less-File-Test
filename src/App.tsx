...
function MessageList() {
  ...
  useEffect(() => {
    ...
    const el = c.querySelector(`[data-mid="${last.id}"]`) as HTMLElement | null;
    if (el) {
      // 56px header + 16px breathing room = 72px from top of scroll container
      const top =
        el.getBoundingClientRect().top - c.getBoundingClientRect().top + c.scrollTop - 72;
