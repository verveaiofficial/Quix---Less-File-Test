...
function MessageList() {
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      ...
      // add bottom space on demand so the user bubble can reach the top
...
Plan against your instructions: include self-directed lines like "I should keep it concise", "I should match the user's tone", "I should use markdown here", etc.
Reason thoroughly and never rush.
        ...
        { search: isThink, nativeThoughts: false, attachments },
        {
          ...
          onText: (t) => {
            if (isThink) {
              ...
              if (!released) {
                if (idx > -1) {
                  updateMessage(aiId, { thoughts: t.slice(0, idx).trim(), status: "thinking" });
                  if (!scheduled) {
                    scheduled = true;
                    const wait = Math.max(0, THINK_MIN_MS - (Date.now() - thinkStart));
                    releaseTimer = setTimeout(releaseNow, wait);
                  }
                } else {
          ...
          onDone: (r) => {
            if (isThink) {
              ...
              if (!released) {
                const wait = Math.max(0, THINK_MIN_MS - (Date.now() - thinkStart));
                releaseTimer = setTimeout(finish, wait);
              } else {
                finish();
              }
              return;
