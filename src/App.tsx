...
function ThinkingStatus({ done, sources, thoughts, thinkTime }: { done: boolean; sources?: SourceItem[]; thoughts?: string; thinkTime?: number }) {
    ...
    <div className={`qts-status visible ${done ? "done" : "active"}`}>
      ...
      <div className="qts-head">
        ...
        <div className="qts-title-row">
          <span className="qts-title">{done ? (finalTime > 0 ? `Thought for ${finalTime}s` : "Thought") : "Thinking"}</span>
          {!done && <span className="qts-meta">{elapsed}s</span>}
...
`;
function AiMessage({ message, mid }: { message: ChatMessage & { thinkTime?: number }; mid: string }) {
  ...
  return (
    <div className="message-ai" data-mid={mid}>
      ...
      <div className="ai-content">
        {isThinkingModel ? (<ThinkingStatus done={message.status !== "thinking"} sources={message.sources} thoughts={message.thoughts} thinkTime={message.thinkTime} />) : (<BubbleIndicator dimmed={isDone} />)}
    ...
    <>
      ...
      <div className="message-scroll" ref={ref}>
        <div className="message-container">
          {messages.map((m) => m.role === "user" ? (<UserMessage key={m.id} mid={m.id} content={m.content} attachments={m.attachments} />) : (<AiMessage key={m.id} mid={m.id} message={m as any} />))}
        </div>
      </div>
    </>
  );
...
.model-item.locked { opacity: 0.55; }
.model-item-content { flex: 1; display: flex; flex-direction: column; gap: 2px; }
.model-title { font-size: 13.5px; font-weight: 500; color: #fff; display: flex; align-items: center; gap: 6px; }
.model-desc { font-size: 11px; color: #9ba1a6; }
.model-check { width: 15px; height: 15px; stroke: #fff; stroke-width: 2.2; flex-shrink: 0; }
.model-lock { width: 12px; height: 12px; stroke: #ff8080; stroke-width: 2; flex-shrink: 0; }
...
.voice-menu { position: absolute; bottom: calc(100% + 10px); right: 0; background: rgba(18,18,22,0.96); border: 1px solid rgba(255,255,255,0.12); border-radius: 14px; width: 200px; padding: 6px 4px; display: flex; flex-direction: column; gap: 4px; backdrop-filter: blur(40px); box-shadow: 0 16px 40px rgba(0,0,0,0.6); z-index: 60; opacity: 0; pointer-events: none; transform: translateY(8px) scale(0.96); transform-origin: bottom right; transition: opacity .22s, transform .22s; }
.voice-menu.show { opacity: 1; pointer-events: all; transform: translateY(0) scale(1); }
.voice-opt { display: flex; align-items: center; gap: 12px; padding: 13px 16px; cursor: pointer; font-size: 14px; color: rgba(255,255,255,0.8); border-radius: 10px; }
.voice-opt:hover { background: rgba(255,255,255,0.08); }
.voice-opt svg { stroke: rgba(255,255,255,0.7); }
`;

const lockSvg = '<svg class="model-lock" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg>';

function ChatInputBar({ onSend, onDeepThinkSend, onDeepThinkStop, onVoiceCall, isDeepThink, dtRunning }: { onSend?: (t: string, a: PendingAttachment[]) => void; onDeepThinkSend?: (t: string) => void; onDeepThinkStop?: () => void; onVoiceCall?: () => void; isDeepThink?: boolean; dtRunning?: boolean }) {
              ...
              <div style={{ position: "relative" }}>
                ...
                {showMic && (
                  <div className={`voice-menu ${voiceMenuOpen ? "show" : ""}`}>
                    ...
                    <div className="voice-opt" onClick={startVoiceType}>
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 1a3 3 0 00-3 3v8a3 3 0 006 0V4a3 3 0 00-3-3z"/><path d="M19 10v2a7 7 0 01-14 0v-2"/><line x1="12" y1="19" x2="12" y2="23"/><line x1="8" y1="23" x2="16" y2="23"/></svg>
                      Voice Type
                    </div>
                  </div>
                )}
...
const vcCSS = `
.voice-call-layer { position: fixed; inset: 0; background: #000; z-index: 9999; }
.voice-call-frame { width: 100%; height: 100%; border: none; }
.voice-call-exit { position: absolute; top: 20px; left: 20px; background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.2); border-radius: 50%; width: 40px; height: 40px; display: flex; align-items: center; justify-content: center; cursor: pointer; color: #fff; z-index: 10000; }
.voice-call-exit:hover { background: rgba(255,255,255,0.15); }
`;
function VoiceCallLayer({ onExit }: { onExit: () => void }) {
  return (
    <>
      <style>{vcCSS}</style>
      <div className="voice-call-layer">
        <button className="voice-call-exit" onClick={onExit} aria-label="Exit voice call">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
        </button>
        <iframe className="voice-call-frame" src="https://quix-voice.vercel.app/" title="Voice Call" />
      </div>
    </>
  );
}
...
`;
function ChatHeader({ hidden }: { hidden?: boolean }) {
...
function MenuDrawer({ hidden }: { hidden?: boolean }) {
...
export default function App() {
  ...
  return (
    <div style={{ height: "100dvh", background: "#000", color: "#fff", overflow: "hidden", position: "relative" }}>
      ...
      <ChatHeader hidden={voiceCallMode} />
      <MenuDrawer hidden={voiceCallMode} />
      <AuthScreen />
      <SettingsPage />
      <CanvasPanel />
      <div className={`qx-layer ${viewMode === "chat" ? "center" : "left"}`}>
        {isDeepThink ? (<DeepThinkLayer frameRef={dtFrameRef} />) : (<MessageList />)}
        <ChatInputBar onSend={handleSend} onDeepThinkSend={handleDeepThinkSend} onDeepThinkStop={handleDeepThinkStop} onVoiceCall={() => setVoiceCallMode(true)} isDeepThink={isDeepThink} dtRunning={dtRunning} />
      </div>
      <div className={`qx-layer ${viewMode === "imagine" ? "center" : "right"}`}>
        <iframe key={imagineNonce} src={IMAGINE_URL} title="Imagine 1.5" />
      </div>
      {loading && <LoadingScreen />}
      {voiceCallMode && <VoiceCallLayer onExit={() => setVoiceCallMode(false)} />}
    </div>
  );
}