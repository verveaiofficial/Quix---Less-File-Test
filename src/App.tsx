...
function MessageList() {
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      ...
      const currentPad = parseFloat(getComputedStyle(container).paddingBottom) || 0;
      if (need > currentPad) container.style.paddingBottom = `${need}px`;

      requestAnimationFrame(() => {
        c.scrollTo({ top: Math.max(0, elTop - 8), behavior: "smooth" });
      });
    });
  }, [messages]);
...
export default function App() {
  ...
  const handleSend = async (text: string, attachments: PendingAttachment[]) => {
    // authoritative guard — no double sends, ever
    ...
    if (isThink) {
      capTimer = setTimeout(() => { if (!released) releaseNow(); }, THINK_MAX_MS);
    }
...
When creating or editing any file (code, txt, md, etc.), always output it inside a fenced code block tagged with its language so it appears in the user's Canvas as a file card with preview and download.`;
        ...
        { search: isThink, nativeThoughts: false, attachments },
        {
          ...
          onDone: (r) => {
            if (isThink) {
              ...
              fullRef.current = r.text || fullRef.current;
              const full = fullRef.current;
              const idx = full.indexOf(THINK_SEP);
              if (idx > -1) {
                updateMessage(aiId, {
      ...
      clearTimeout(capTimer);
      const cur = useChatStore.getState().messages.find((m) => m.id === aiId);
      if (!cur) { setIsSending(false); return; }
      if (cur.status === "thinking") {
        // stream ended without releasing — never leave her frozen
        updateMessage(aiId, { content: cur.content?.trim() ? cur.content : "Quix lost her train of thought. Send it again.", status: "error" });
        setIsSending(false);
      } else if (cur.status === "error") {
        setIsSending(false);
      }
      // "streaming" → AiMessage finalize clears isSending
    }
  };
