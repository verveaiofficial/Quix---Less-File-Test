...
import { CanvasPanel, useCanvasStore, downloadFile, extFor } from "./canvas";
    ...
    <div className="code-block">
      <div className="code-head">
        ...
        <span>
          ...
          <button onClick={() => downloadFile(`quix-file.${extFor(lang)}`, code, lang)}>Download</button>
        </span>
      </div>
      <pre>{code}</pre>
    </div>
...
.upload-opt .on-dot { margin-left: auto; width: 7px; height: 7px; border-radius: 50%; background: #39ff14; }
              ...
              {!isDeepThink && (
                <div style={{ position: "relative" }}>
                  ...
                  <div className={`pop-menu ${menuOpen ? "show" : ""}`} style={{ width: 180 }}>
                    ...
                    <div className="upload-opt" onClick={(e) => { e.stopPropagation(); setCanvasOn(!canvasOn); }} onMouseDown={prevent}>
                      ...
                      Canvas
                      {canvasOn && <span className="on-dot" />}
                    </div>
                  </div>
                </div>
              )}
  ...
  return (
    <>
      ...
      <div id="chat-options-menu" className={optOpen ? "show" : ""}>
        <div className="chat-opt shimmer-btn" onClick={(e) => { e.stopPropagation(); shimmer(e); setOptOpen(false); resetChat(); }}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" /></svg>
          New chat
        </div>
        <div className="chat-opt shimmer-btn" onClick={(e) => { e.stopPropagation(); shimmer(e); setOptOpen(false); if (currentChatId) togglePin(currentChatId); }}>
        ...
        <div className="chat-opt shimmer-btn" onClick={(e) => { e.stopPropagation(); shimmer(e); setOptOpen(false); setRenameVal(chatTitle); setRenameOpen(true); }}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7" /><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z" /></svg>
          Rename chat
        </div>
        <div className="chat-opt danger shimmer-btn" onClick={(e) => { e.stopPropagation(); shimmer(e); setOptOpen(false); if (session && currentChatId) deleteChat(currentChatId); resetChat(); }}>
        ...
        <div className="drawer-inner">
          ...
          <div className="drawer-footer">
            {session ? (
              ...
              <button className="signin-btn shimmer-btn" onClick={(e) => { shimmer(e); setDrawerOpen(false); setTimeout(() => openAuthFromDrawer(), 150); }}>
            ...
            <div className="profile-row">
              <button className="signin-btn shimmer-btn" style={{ flex: 1 }} onClick={(e) => { shimmer(e); setDrawerOpen(false); setTimeout(() => openSettingsFromDrawer(), 150); }}>
              ...
              <button className="settings-circle shimmer-btn" onClick={(e) => { shimmer(e); setDrawerOpen(false); setTimeout(() => openSettingsFromDrawer(), 150); }} aria-label="Settings">
...
When your reasoning is complete, write a line containing exactly "${THINK_SEP}", then give the final answer in clean markdown.`;
      ...
      if (useCanvasStore.getState().on) {
        prompt += `\n\n--- Canvas mode ---
When creating or editing any file (code, txt, md, etc.), always output it inside a fenced code block tagged with its language so it appears in the user's Canvas with preview and download.`;
      }
