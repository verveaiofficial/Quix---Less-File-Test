...
import {
  MODELS, CHAT_MODELS, LIMITS, IMAGINE_URL, APP_VERSION, THINK_SEP,
  ...
  askGeminiStream, abortGemini, buildPrompt, runDailyMemorySync,
  stripObs, extractObs, saveObservation,
} from "./core";
import { CanvasPanel, FileCard, InlineCodeBlock, useCanvasStore, extractFiles } from "./canvas";

/* local utils (so a truncated core.tsx can't break the app) */
function useStreamText(full: string, active: boolean, speed = 10, step = 2): string {
...
const CHAIN: Record<string, string[]> = {
  ...
  deepthink: ["deepthink", "thinking", "flash", "lite"], coder: ["coder", "flash", "lite"],
};
...
function ChatInputBar({ onSend }: { onSend?: (t: string, a: PendingAttachment[]) => void }) {
  ...
  const pickModel = (id: string) => {
    if (isGuest && id !== "thinking") { setModelMenuOpen(false); useUIStore.getState().openAuth(); return; }
    setActiveModel(id); setModelMenuOpen(false);
  };

  const send = () => { const text = inputValue.trim(); if (!text) return; if (attachments.length === 0 && !text) return; onSend?.(text, attachments); setInputValue(""); setAttachments([]); if (taRef.current) { taRef.current.blur(); taRef.current.style.height = "40px"; } };
    ...
    <>
      ...
      <div className="input-wrapper" style={{ bottom: `${bottomOffset}px` }}>
        <div className="input-bar">
          ...
          <textarea ref={taRef} placeholder="Ask Quix..." rows={1} value={inputValue} onChange={(e) => { setInputValue(e.target.value); if (taRef.current) { taRef.current.style.height = "40px"; taRef.current.style.height = Math.min(taRef.current.scrollHeight, 250) + "px"; } }} onBlur={() => { if (!menuOpen) setBottomOffset(0); }} />
          <div className="action-row">
            ...
            <div className="action-right">
              {canvasOn && (<button type="button" className="cv-pill" onClick={() => openFilesList()}><svg width="13" height="13" viewBox="0 0 24 24" fill="none" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 19a2 2 0 01-2 2H4a2 2 0 01-2-2V5a2 2 0 012-2h5l2 3h9a2 2 0 012 2z" /></svg>Canvas{fileCount > 0 ? ` · ${fileCount}` : ""}</button>)}
