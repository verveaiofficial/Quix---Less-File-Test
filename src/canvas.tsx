...
export const useCanvasStore = create<any>((set) => ({
  ...
  tab: "code" as "code" | "preview" | "edit",
  overrides: {} as Record<string, string>,
  setOn: (v: boolean) => set({ on: v }),
  setOpen: (v: boolean) => set({ open: v }),
  setSelId: (id: string | null) => set({ selId: id }),
  setTab: (t: "code" | "preview" | "edit") => set({ tab: t }),
  openFile: (id: string, tab?: "code" | "preview" | "edit") =>
    set({ open: true, selId: id, ...(tab ? { tab } : {}) }),
...
/* ================= PANEL (sheet only, tab from store) ================= */
export function CanvasPanel() {
  const { open, setOpen, selId, setSelId, tab, setTab, overrides, setOverride } = useCanvasStore();
          ...
          {!sel ? (
            ...
            <>
              ...
              <div className="cv-view">
                ...
                {tab === "preview" && canPreview && extFor(sel.lang) === "html" && <iframe title="preview" srcDoc={code} />}
                {tab === "preview" && canPreview && extFor(sel.lang) === "md" && (
