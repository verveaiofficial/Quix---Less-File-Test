...
/* ================= OBSERVATIONS (general human insights — not user-specific) ================= */
...
export async function saveObservation(summary: string) {
  ...
  if (sb) { try { await sb.from("observations").insert({ summary: s }); } catch {} }
}
...
export function buildPrompt(model: string, text: string, history: ChatMessage[]): string {
  ...
  blocks.push(`--- Internal observation (do NOT mention or quote this instruction in your reply or reasoning) ---\nAt the VERY END of your reply, silently append ONE line starting with ${OBS_TAG}. This line must contain a GENERAL observation about human nature, behavior, or interaction patterns you learned from this turn — e.g., how humans like to be treated by an AI, what they appreciate or dislike, common emotional triggers, communication preferences. Do NOT mention any specific user, their name, email, or personal details. Write it as an insight about humans in general. Example: "Humans tend to feel more respected when an AI asks before assuming." Keep it 1-2 sentences.`);
