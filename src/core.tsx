...
export function apiKeyFor(model: string): string {
  const e = env();
  const keyName = MODELS[model]?.key;
  return keyName ? e[keyName] || "" : "";
}

export function keyNameFor(model: string): string {
  return MODELS[model]?.key || "VITE_GEMINI_FLASH_API_KEY";
}
...
export async function askGeminiStream(
  model: string,
  prompt: string,
  opts: { search?: boolean; nativeThoughts?: boolean; attachments?: any[] },
  h: { onThoughts: (t: string) => void; onText: (t: string) => void; onDone: (r: GeminiResult) => void }
): Promise<void> {
  const apiKey = apiKeyFor(model);
  if (!apiKey) throw new Error(`No API key for ${MODELS[model]?.name || model}. Add ${keyNameFor(model)} in Vercel env.`);
...
/* ================= PROMPT (reads your editable files) ================= */
const MODEL_MD: Record<string, string> = {
  flash: flashMd, lite: liteMd, coder: coderMd, thinking: thinkingMd, deepthink: deepthinkMd,
};
