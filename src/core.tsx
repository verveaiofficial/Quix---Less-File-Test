...
// NO APP-LEVEL QUOTAS. Gemini keys are unlimited. Only guest lock on thinking.
function readGuestUsage(): number { try { return JSON.parse(localStorage.getItem("quix_guest_thinking_" + dayKey()) || "0"); } catch { return 0; } }
function writeGuestUsage(n: number) { try { localStorage.setItem("quix_guest_thinking_" + dayKey(), String(n)); } catch {} }

export const useUsageStore = create<any>((set, get) => ({
  guestThinkingUsed: readGuestUsage(),
  canGuestThink: () => readGuestUsage() < GUEST_THINKING_LIMIT,
  consumeGuestThinking: () => { const n = readGuestUsage() + 1; writeGuestUsage(n); set({ guestThinkingUsed: n }); },
  // Signed-in users: unlimited. Guests: only thinking, 3/day.
  resolve: (m: string) => {
    const sess = useAuthStore.getState().session;
    if (sess) return m; // signed in = always allowed
    if (m === "thinking" && get().canGuestThink()) return "thinking";
    return null; // guest trying non-thinking, or out of free thinking
  },
}));
...
export async function askGeminiStream(model: string, prompt: string, opts: { search?: boolean; nativeThoughts?: boolean; attachments?: any[] }, h: { onThoughts: (t: string) => void; onText: (t: string) => void; onDone: (r: GeminiResult) => void }): Promise<void> {
  ...
  if (!apiKey) throw new Error(`Missing ${MODELS[model]?.key} in Vercel env vars.`);
  ...
  try {
    ...
    if (!res.ok || !res.body) { const d = await res.json().catch(() => null); throw new Error(d?.error?.message || `Gemini error (${res.status})`); }
