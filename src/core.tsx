...
export async function runDailyMemorySync() {
  ...
  try {
    ...
    let transcript = "";
    for (const c of chats) { const { data: msgs } = await sb.from("messages").select("role,content").eq("chat_id", c.id).order("created_at", { ascending: true }).limit(20); if (msgs?.length) transcript += `# ${c.title}\n` + msgs.map((m: any) => `${m.role}: ${m.content}`).join("\n") + "\n\n"; }
...
export function loadLocalMessages(): ChatMessage[] {
  try { const arr = JSON.parse(localStorage.getItem(LOCAL_MSG_KEY) || "[]"); return Array.isArray(arr) ? arr : []; } catch { return []; }
}
export function clearLocalMessages() { try { localStorage.removeItem(LOCAL_MSG_KEY); } catch {} }
...
export function saveObservationLocal(summary: string) {
  try { const arr = JSON.parse(localStorage.getItem(OBS_LOCAL_KEY) || "[]"); arr.push({ t: Date.now(), s: summary }); localStorage.setItem(OBS_LOCAL_KEY, JSON.stringify(arr.slice(-300))); } catch {}
}
