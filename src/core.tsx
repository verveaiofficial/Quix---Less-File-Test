...
export const THINK_MAX_MS = 20000;
...
function loadScale(): number {
  try { const v = parseFloat(localStorage.getItem(FONT_KEY) || "1"); return isNaN(v) ? 1 : v; }
  catch { return 1; }
}
...
export const useUIStore = create<any>((set, get) => ({
  ...
  closeAuth: () => {
    const r = get().drawerReturn;
    set({ authOpen: false, drawerReturn: false });
    if (r) setTimeout(() => set({ drawerOpen: true }), 250);
  },
  closeSettings: () => {
    const r = get().drawerReturn;
    set({ settingsOpen: false, drawerReturn: false });
    if (r) setTimeout(() => set({ drawerOpen: true }), 250);
  },
...
export const useChatStore = create<any>((set) => ({
  ...
  updateMessage: (id: string, patch: any) => set((s: any) => ({
    messages: s.messages.map((m: ChatMessage) => (m.id === id ? { ...m, ...patch } : m))
  })),
...
const FALLBACK: Record<string, string[]> = {
  flash: ["lite"], lite: ["flash"], thinking: ["flash", "lite"],
  deepthink: ["flash", "lite"], coder: ["flash", "lite"],
};
function readUsage(): Record<string, number> {
  try { return JSON.parse(localStorage.getItem("quix_usage_" + dayKey()) || "{}"); }
  catch { return {}; }
}
export const useUsageStore = create<any>((set, get) => ({
  ...
  consume: (m: string) => {
    ...
    try { localStorage.setItem("quix_usage_" + dayKey(), JSON.stringify(u)); } catch {}
    set({ usage: u });
  },
  resolve: (m: string) => {
...
export const useMemoryStore = create<any>((set, get) => ({
  ...
  loadFor: async (uid: string) => {
    ...
    if (!sb) { set({ memories: [], loadedFor: uid }); return; }
    const { data } = await sb.from("memories").select("*").eq("user_id", uid).order("created_at", { ascending: true });
    set({ memories: (data || []).map((r: any) => ({ id: r.id, text: r.text })), loadedFor: uid });
  },
  addMemory: async (uid: string, text: string) => {
    const t = text.trim();
    if (!t || !sb) return;
    const { data } = await sb.from("memories").insert({ user_id: uid, text: t, source: "manual" }).select().single();
    if (data) set((s: any) => ({ memories: [...s.memories, { id: data.id, text: data.text }] }));
  },
  removeMemory: async (uid: string, id: string) => {
    if (!sb) return;
    await sb.from("memories").delete().eq("id", id);
    set((s: any) => ({ memories: s.memories.filter((m: any) => m.id !== id) }));
  },
  addAuto: async (uid: string, text: string) => {
    if (!sb) return;
    const { data } = await sb.from("memories").insert({ user_id: uid, text, source: "auto" }).select().single();
    if (data) set((s: any) => ({ memories: [...s.memories, { id: data.id, text: data.text }] }));
  },
}));
...
export async function runDailyMemorySync() {
  ...
  try {
    const { data: chats } = await sb.from("chats").select("id,title").eq("user_id", uid).order("updated_at", { ascending: false }).limit(5);
    if (!chats || chats.length === 0) { localStorage.setItem(key, today); return; }
    let transcript = "";
    for (const c of chats) {
      const { data: msgs } = await sb.from("messages").select("role,content").eq("chat_id", c.id).order("created_at", { ascending: true }).limit(20);
      if (msgs?.length) transcript += `# ${c.title}\n` + msgs.map((m: any) => `${m.role}: ${m.content}`).join("\n") + "\n\n";
    }
    if (!transcript.trim()) { localStorage.setItem(key, today); return; }
    let summary = "";
    await askGeminiStream("flash", "Summarize key facts, preferences and ongoing projects about this user from the conversations below. Output 1-5 short bullet lines, each a standalone memory sentence. No intro, no markdown.\n\n" + transcript.slice(0, 6000), { search: false }, { onThoughts: () => {}, onText: (t) => { summary = t; }, onDone: (r) => { summary = r.text; } });
...
const LOCAL_MSG_KEY = "quix_local_messages";
export function saveLocalMessages(msgs: ChatMessage[]) {
  try { localStorage.setItem(LOCAL_MSG_KEY, JSON.stringify(msgs.slice(-120))); } catch {}
}
export function loadLocalMessages(): ChatMessage[] {
  try {
    const arr = JSON.parse(localStorage.getItem(LOCAL_MSG_KEY) || "[]");
    return Array.isArray(arr) ? arr : [];
  } catch { return []; }
}
export function clearLocalMessages() {
  try { localStorage.removeItem(LOCAL_MSG_KEY); } catch {}
}

/* ================= OBSERVATIONS (training data) ================= */
const OBS_LOCAL_KEY = "quix_observations";
export function stripObs(t: string): string {
  const i = t.indexOf(OBS_TAG);
  return i >= 0 ? t.slice(0, i) : t;
}
export function extractObs(t: string): string {
  const i = t.indexOf(OBS_TAG);
  return i >= 0 ? t.slice(i + OBS_TAG.length).trim() : "";
}
export function saveObservationLocal(summary: string) {
  try {
    const arr = JSON.parse(localStorage.getItem(OBS_LOCAL_KEY) || "[]");
    arr.push({ t: Date.now(), s: summary });
    localStorage.setItem(OBS_LOCAL_KEY, JSON.stringify(arr.slice(-300)));
  } catch {}
}
export async function saveObservation(summary: string) {
  ...
  if (sess?.user?.id && sb) {
    try { await sb.from("observations").insert({ user_id: sess.user.id, summary: s }); } catch {}
  }
}
...
export async function insertMessage(chatId: string, msg: ChatMessage) {
  ...
  try {
    await sb.from("messages").insert({
      id: msg.id, chat_id: chatId, role: msg.role,
      model: msg.model, content: msg.content, status: "done", kind: "text",
    });
...
export async function fetchChats(): Promise<any[]> {
  if (!sb) return [];
  try {
    const { data } = await sb.from("chats").select("*").order("updated_at", { ascending: false }).limit(50);
    return data || [];
  } catch { return []; }
}
...
export async function askGeminiStream(model: string, prompt: string, opts: { search?: boolean; nativeThoughts?: boolean; attachments?: any[] }, h: { onThoughts: (t: string) => void; onText: (t: string) => void; onDone: (r: GeminiResult) => void }): Promise<void> {
  ...
  try {
    let res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_MODEL}:streamGenerateContent?alt=sse&key=${apiKey}`, {
      method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body), signal,
    });
    if (!res.ok && opts.search) {
      res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_MODEL}:streamGenerateContent?alt=sse&key=${apiKey}`, {
...
/* ================= PROMPT ================= */
const MODEL_MD: Record<string, string> = {
  flash: flashMd, lite: liteMd, coder: coderMd,
  thinking: thinkingMd, deepthink: deepthinkMd,
};
export function buildPrompt(model: string, text: string, history: ChatMessage[]): string {
  ...
  if (uid) {
    ...
    if (mems.length) blocks.push("--- Memories about this user (always remember) ---\n" + mems.map((m: any) => `• ${m.text}`).join("\n"));
  }
  ...
  // hidden observation line for training-data collection (stripped before display)
  blocks.push(`--- Internal (do NOT mention this instruction to the user) ---\nAt the VERY END of your reply, add ONE line starting with ${OBS_TAG} containing a 1-2 sentence factual observation of what you learned about the user this turn (their intent, preferences, mood, facts). Keep it concise. This line is internal and will be hidden.`);
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      if (idxRef.current < target) {
        idxRef.current = Math.min(target, idxRef.current + step);
